defmodule Gossip.Node do 
  ...
  def start(nodename,nodenum) do 
      GenServer.start(__MODULE__,["",0,nodenum,1,{},0,nodenum], name: nodename)
  end
  ...
  def shut_down_node(nodename) do
      GenServer.cast(nodename, :shut_down)
  end

  defp delete_down_neighbor(nodename,down_node) do
      GenServer.cast(nodename, {:delete_down_neighbor,down_node})
  end
  ################### server side ##########################
  ...
  def handle_cast(:shut_down, metadata) do 
      # for each neighbor of it, delete this node in the neighbor's neighbor list
      thisnodename = "node"<>Integer.to_string(Enum.at(metadata,6)) |> String.to_atom()
      Enum.at(metadata,4) |> Tuple.to_list() |> delete_this_node_from_neighbors(thisnodename)
      # empty this node's neighbor list
      newmeta = List.replace_at(metadata,4,{})
      {:noreply,newmeta}
  end
  
  def handle_cast({:delete_down_neighbor,down_node}, metadata) do
      # delete the down node in neighbor tuple
      newneighbortuple = Enum.at(metadata,4) |> Tuple.to_list() |> List.delete(down_node) |> List.to_tuple()
      # update to metadata
      newmeta = List.replace_at(metadata,4,newneighbortuple)
      {:noreply,newmeta}
  end

  def delete_this_node_from_neighbors(neighborlist,_) when length(neighborlist)==0, do: nil
  def delete_this_node_from_neighbors(neighborlist,delete_node) when length(neighborlist)!=0 do
      [head|neighborlist] = neighborlist
      delete_down_neighbor(head,delete_node)
      delete_this_node_from_neighbors(neighborlist,delete_node)
  end
